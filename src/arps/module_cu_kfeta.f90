...
! MODIFICATIONS:
!
! Fanyou Kong, April 2002
! Remove subroutine KF_eta_CPS (ARPS doesn't need), and pass ARPS input
! parameter 'kffbfct' (feedback factor) and 'kfsubsattrig' (sub-
! saturation option) into KF_eta_PARA
!
!------------------------------------------------------------------------
...
CONTAINS
   ...
   SUBROUTINE KF_eta_PARA (I, J,                           &
                      ...
                      RAINCV,NCA,NTST,kffbfct,kfsubsattrig, &
...
!-----------------------------------------------------------
      ...
      INTEGER, INTENT(IN   ) :: kfsubsattrig   ! passed-in by ARPS
...
!
      ...
      REAL,  INTENT(IN   ) :: kffbfct          ! passed-in by ARPS
!
...
!
   ...
   REAL    :: QSLCL,RHLCL,DQSSDT    ! ARPS requires (by kfsubsattrig)
...
!...!!!!!! for MM5, SET DTRH = 0. !!!!!!!!
          DTRH = 0.
          IF(kfsubsattrig == 1) THEN  ! ARPS pass-in kfsubsattrig to
                                      ! activate sub-saturation
!           U00 = 0.75
!           IF(U00.lt.1.)THEN
...
!3004       FORMAT( ,No Downdraft!;  I=,I3,2X,J=,I3,ISHALL =,I2)
        ...
        ELSE
         ...
         CPR=TRPPT
!         PPTFLX = TRPPT-TDER
         PPTFLX = amax1(TRPPT-TDER,0.0)   ! F.KONG (2002)
         PEFF=PPTFLX/TRPPT
         IF(IPRNT)THEN
