...
! DTD 01/23/07: it appears this subroutine is no longer called by anything
! and is superseded by the driver subroutine in micro_wsm6.f90
...
!-------------------------------------------------------------------
      ...
      DO j=jts,jte
         ...
         ENDDO
         ENDDO
!         CALL wsm62D_WRF(t, q(ims,kms,j), qci, qrs,                &
...
!
  SUBROUTINE wsm62D_WRF(t, q, qci, qrs, w, den, p,                 &
                     delz, rain, rainncv,delt,                     &
                     lat,                                          &
                     ids,ide, jds,jde, kds,kde,                    &
                     ims,ime, jms,jme, kms,kme,                    &
                     its,ite, jts,jte, kts,kte,mpteqnterms_2D)
!-------------------------------------------------------------------
  IMPLICIT NONE
!-------------------------------------------------------------------
  INTEGER,      INTENT(IN   )    ::   ids,ide, jds,jde, kds,kde , &
...
! LOCAL VAR
  ...
  ! DTD: added array to store mp temp eqn terms
  REAL, DIMENSION( its:ite, kts:kte, 27) :: mpteqnterms_2D
!
!=================================================================
...
!
! DTD: initialize mpteqnterms_2D array
      mpteqnterms_2D = 0.0
...
!---------------------------------------------------------------
              ...
              if(qrs(i,k,2).gt.0.) then
                ...
                ! DTD: add this to the snow melting rate portion of the
                ! array

                mpteqnterms_2D(i,k,8) = -psml(i,k)*mstep(i)/dtcld
...
!---------------------------------------------------------------
              if(qrs(i,k,3).gt.0.) then
                ...
                ! DTD: add this to the graupel melting rate portion of the
                ! array

                mpteqnterms_2D(i,k,9) = -pgml(i,k)*mstep(i)/dtcld
      ...
      do k = kts, kte
        do i = its, ite
          ...
          if(supcol.lt.0.and.qci(i,k,2).gt.0.) then
            ...
            !DTD: store cloud ice melting rate
            mpteqnterms_2D(i,k,7) = qci(i,k,2)/dtcld
...
!---------------------------------------------------------------
          if(supcol.gt.40..and.qci(i,k,1).gt.0.) then
            ...
            !DTD: store cloud water freezing rate
            mpteqnterms_2D(i,k,18) = qci(i,k,1)/dtcld
            qci(i,k,1) = 0.
          endif
!---------------------------------------------------------------
...
!---------------------------------------------------------------
          if(supcol.gt.0..and.qci(i,k,1).gt.qmin) then
            ...
            *den(i,k)/denr/xncr*qci(i,k,1)*qci(i,k,1)*dtcld,qci(i,k,1))
            ...
            !DTD: store cloud water freezing rate
            mpteqnterms_2D(i,k,18) = pfrzdtc/dtcld
...
!---------------------------------------------------------------
          if(supcol.gt.0..and.qrs(i,k,1).gt.0.) then
            ...
            !DTD: store rain freezing rate
            mpteqnterms_2D(i,k,23) = pfrzdtr/dtcld
          endif
        enddo
      enddo
!
...
!     update
        ...
        ! DTD: Store rain evaporation rate and add enhanced melting
        ! rates of snow and graupel to the already-stored melting rates
        ! Also add evaporation of melting graupel and snow to the rain
        ! evaporation rate.

        !sublimation/deposition of qi
        IF(t(i,k).lt.t0c) THEN
          IF(pisd(i,k) <= 0.0) THEN
            mpteqnterms_2D(i,k,3) = -pisd(i,k)
          ELSE
            mpteqnterms_2D(i,k,14) = pisd(i,k)
          END IF
          !sublimation/deposition of qs

          IF(pres(i,k,2) <= 0.0) THEN
            mpteqnterms_2D(i,k,4) = -pres(i,k,2)
          ELSE
            mpteqnterms_2D(i,k,15) = pres(i,k,2)
          END IF

          !sublimation/deposition of qg

          IF(pres(i,k,3) <= 0.0) THEN
            mpteqnterms_2D(i,k,5) = -pres(i,k,3)
          ELSE
            mpteqnterms_2D(i,k,16) = pres(i,k,3)
          END IF

          IF(pres(i,k,1) <= 0.0) THEN
            mpteqnterms_2D(i,k,2) = -(pres(i,k,1) + psev(i,k) + pgev(i,k))
          ELSE
            mpteqnterms_2D(i,k,2) = -(psev(i,k) + pgev(i,k))
            mpteqnterms_2D(i,k,12) = pres(i,k,1)
          END IF

          mpteqnterms_2D(i,k,13) = pgen(i,k)
          mpteqnterms_2D(i,k,13) = pgen(i,k)
          mpteqnterms_2D(i,k,20) = psacw(i,k)
          mpteqnterms_2D(i,k,21) = pgacw(i,k)
          mpteqnterms_2D(i,k,24) = piacr(i,k)
          mpteqnterms_2D(i,k,25) = psacr(i,k)
          mpteqnterms_2D(i,k,26) = pgacr(i,k)

        ELSE

          IF(pres(i,k,1) <= 0.0) THEN
            mpteqnterms_2D(i,k,2) = -pres(i,k,1)
          ELSE
            mpteqnterms_2D(i,k,12) = pres(i,k,1)
          END IF

          mpteqnterms_2D(i,k,8) = mpteqnterms_2D(i,k,8) - pseml(i,k)
          mpteqnterms_2D(i,k,9) = mpteqnterms_2D(i,k,9) - pgeml(i,k)
...
!
      do k = kts, kte
        do i = its, ite
          ...
          ! DTD: store evaporation rate of cloud water
          IF(pcon(i,k) < 0.0) THEN
            mpteqnterms_2D(i,k,1) = -pcon(i,k)
          ELSE
            mpteqnterms_2D(i,k,11) = pcon(i,k)
          END IF
...
!===================================================================
!
! Simplified gamma distribution constraint for rain
!
  SUBROUTINE wsm62D_GR(t, q, qci, qrs, w, den, p,                  &
                     delz, rain, rainncv,delt,                     &
                     lat,                                          &
                     ids,ide, jds,jde, kds,kde,                    &
                     ims,ime, jms,jme, kms,kme,                    &
...
!
!----------------------------------------------------------------
!     simplified constrained gamma (S-C-G) raindrop size distribution
!        for rain microphysical parameterization ,
!        added for S-C-G parameterization on 3/29/05
!     valid for rain water content W=x*y > 1.e-6 g m^-3.
!
!       find C-G dsd parameters
           lw(x,y)=log10(x*y*1e+3)
           revp1(x,y)= 10.0**(0.003091*lw(x,y)**4         &
                           + 0.03695*lw(x,y)**3                &
                    + 0.1199*lw(x,y)**2                 &
                    + 0.9931*lw(x,y) +0.8019)
           revp2(x,y)= 10.0**(0.000604*lw(x,y)**4         &
                           + 0.005344*lw(x,y)**3                &
                    - 0.002943*lw(x,y)**2                 &
                    + 0.8755*lw(x,y) -0.8173)
           racr(x,y)= 10.0**(-0.0000603*lw(x,y)**4         &
                    - 0.00255*lw(x,y)**3                &
                    - 0.0212*lw(x,y)**2                 &
                         + 0.93304*lw(x,y) -2.294)
           rvtm(x,y)= (-4.509e-4*lw(x,y)**4                 &
                        + 0.0148*lw(x,y)**3                        &
                   + 0.263*lw(x,y)**2                         &
                        + 1.410*lw(x,y) + 5.7992)
!
...
!---------------------------------------------------------------
! pracw: accretion of cloud water by rain [LFO 51]
!        (C->R) S-C-G Modification on 3/29/05
!---------------------------------------------------------------
          if(qrs(i,k,1).gt.10.*qcrmin.and.qci(i,k,1).gt.qmin) then
            pracw(i,k) = min(racr(qrs(i,k,1),den(i,k))                   &
                        *qci(i,k,1)*denfac(i,k),qci(i,k,1)/dtcld)
          endif
!---------------------------------------------------------------
! pres1: evaporation/condensation rate of rain [HDC 14]
!        (V->R or R->V)
!        S-C-G Modification on 3/29/05
!---------------------------------------------------------------
          if(qrs(i,k,1).gt.10.*qcrmin) then
            coeres = rslope2(i,k,1)*sqrt(rslope(i,k,1)*rslopeb(i,k,1))
            pres(i,k,1) = (rh(i,k,1)-1.)*(revp1(qrs(i,k,1),den(i,k))   &
                      +revp2(qrs(i,k,1),den(i,k))*work2(i,k))/work1(i,k,1)
            if(pres(i,k,1).lt.0.) then
              pres(i,k,1) = max(pres(i,k,1),-qrs(i,k,1)/dtcld)
              pres(i,k,1) = max(pres(i,k,1),satdt/2)
            else
...
!
          ...
          diameter  = min(dicon * sqrt(xmi),dimax)
          if(qrs(i,k,1).gt.10.*qcrmin)then
            vt2r=rvtm(qrs(i,k,1),den(i,k))*denfac(i,k) ! S-C-G Modification on 3/29/05
          else
            vt2r=pvtr*rslopeb(i,k,1)*denfac(i,k)
          endif
          vt2s=pvts*rslopeb(i,k,2)*denfac(i,k)
...
!        S-C-G Modification on 3/29/05
!-------------------------------------------------------------
              praci(i,k) = racr(qrs(i,k,1),den(i,k))                        &
                          *qci(i,k,2)*denfac(i,k)
              praci(i,k) = min(praci(i,k),qci(i,k,2)/dtcld)
!-------------------------------------------------------------
! piacr: Accretion of rain by cloud ice [LFO 26]
...
!
      ...
      enddo
      enddo                  ! big loops
  END SUBROUTINE wsm62d_GR

!===================================================================
!
! Diagnostic N0
!
  SUBROUTINE wsm62D_N0(t, q, qci, qrs, w, den, p,                  &
                     delz, rain, rainncv, delt,                    &
                     lat,                                          &
                     ids,ide, jds,jde, kds,kde,                    &
                     ims,ime, jms,jme, kms,kme,                    &
...
!     valid for mixing ratio > 1.e-9 kg/kg.
!------------------------------------------------------
!     modified on March 6, 2006 by Zhang with a diagnostic N0
      dn0r(x,y)=7835.5*1000*(x*y*1000)**0.681
!--------------------------------------------------------
      lamdar(x,y)=(pidn0r*dn0r(x,y)/(x*y))**.25
      lamdas(x,y,z)=(pidn0s*z/(x*y))**.25
      lamdag(x,y)=(pidn0g/(x*y))**.25
!
!----------------------------------------------------------------
...
!---------------------------------------------------------------
          if(supcol.gt.0..and.qrs(i,k,1).gt.0.) then
            ...
            pfrzdtr = min(20.*pi**2*pfrz1*dn0r(den(i,k),qrs(i,k,1))     &
                    *denr/den(i,k)*rslope3(i,k,1)**2*rslope(i,k,1)      &
                    * (exp(pfrz2*supcol)-1.)*dtcld,qrs(i,k,1))
            qrs(i,k,3) = qrs(i,k,3) + pfrzdtr
            t(i,k) = t(i,k) + xlf/cpm(i,k)*pfrzdtr
            qrs(i,k,1) = qrs(i,k,1)-pfrzdtr
...
!---------------------------------------------------------------
          if(qrs(i,k,1).gt.qcrmin.and.qci(i,k,1).gt.qmin) then
            pracw(i,k) = min(pacrr*dn0r(den(i,k),qrs(i,k,1))       &
                             *rslope3(i,k,1)*rslopeb(i,k,1)        &
                        *qci(i,k,1)*denfac(i,k),qci(i,k,1)/dtcld)
          endif
!---------------------------------------------------------------
! pres1: evaporation/condensation rate of rain [HDC 14]
...
!---------------------------------------------------------------
          if(qrs(i,k,1).gt.0.) then
            coeres = rslope2(i,k,1)*sqrt(rslope(i,k,1)*rslopeb(i,k,1))
            pres(i,k,1) = (rh(i,k,1)-1.)*(precr1*dn0r(den(i,k),qrs(i,k,1))*rslope2(i,k,1)   &
                         +precr2*dn0r(den(i,k),qrs(i,k,1))*work2(i,k)*coeres)/work1(i,k,1)
            if(pres(i,k,1).lt.0.) then
              pres(i,k,1) = max(pres(i,k,1),-qrs(i,k,1)/dtcld)
              pres(i,k,1) = max(pres(i,k,1),satdt/2)
            else
...
!-------------------------------------------------------------
!--- original
!              praci(i,k) = pacrr*denr(i,k)*rslope3(i,k,1)*rslopeb(i,k,1)      &
!                          *qci(i,k,2)*denfac(i,k)
!--- wyh
              praci(i,k) = pacrr*dn0r(den(i,k),qrs(i,k,1))*rslope3(i,k,1)*rslopeb(i,k,1)      &
                          *qci(i,k,2)*denfac(i,k)
              praci(i,k) = min(praci(i,k),qci(i,k,2)/dtcld)
!-------------------------------------------------------------
! piacr: Accretion of rain by cloud ice [LFO 26]
!        (T<T0: R->S or R->G)
!-------------------------------------------------------------
              piacr(i,k) = pi**2*avtr*dn0r(den(i,k),qrs(i,k,1))*denr*xni(i,k)*denfac(i,k)     &
                          *g6pbr*rslope3(i,k,1)*rslope3(i,k,1)          &
                          *rslopeb(i,k,1)/24./den(i,k)
              piacr(i,k) = min(piacr(i,k),qrs(i,k,1)/dtcld)
            endif
          ...
          if(qrs(i,k,2).gt.qcrmin.and.qrs(i,k,1).gt.qcrmin) then
            if(supcol.gt.0) then
              acrfac = 5.*rslope3(i,k,2)*rslope3(i,k,2)*rslope(i,k,1)   &
                      +2.*rslope3(i,k,2)*rslope2(i,k,2)*rslope2(i,k,1)  &
                      +.5*rslope2(i,k,2)*rslope2(i,k,2)*rslope3(i,k,1)
              pracs(i,k) = pi**2*dn0r(den(i,k),qrs(i,k,1))*n0s*n0sfac(i,k)*abs(vt2r-vt2s)     &
                          *(dens/den(i,k))*acrfac
              pracs(i,k) = min(pracs(i,k),qrs(i,k,2)/dtcld)
            endif
!-------------------------------------------------------------
...
!-------------------------------------------------------------
            acrfac = 5.*rslope3(i,k,1)*rslope3(i,k,1)*rslope(i,k,2)     &
                    +2.*rslope3(i,k,1)*rslope2(i,k,1)*rslope2(i,k,2)    &
                    +.5*rslope2(i,k,1)*rslope2(i,k,1)*rslope3(i,k,2)
            psacr(i,k) = pi**2*dn0r(den(i,k),qrs(i,k,1))*n0s*n0sfac(i,k)*abs(vt2s-vt2r)       &
                        *(denr/den(i,k))*acrfac
            psacr(i,k) = min(psacr(i,k),qrs(i,k,1)/dtcld)
          endif
!-------------------------------------------------------------
...
!-------------------------------------------------------------
          if(qrs(i,k,3).gt.qcrmin.and.qrs(i,k,1).gt.qcrmin) then
            acrfac = 5.*rslope3(i,k,1)*rslope3(i,k,1)*rslope(i,k,3)     &
                    +2.*rslope3(i,k,1)*rslope2(i,k,1)*rslope2(i,k,3)    &
                    +.5*rslope2(i,k,1)*rslope2(i,k,1)*rslope3(i,k,3)
            pgacr(i,k) = pi**2*dn0r(den(i,k),qrs(i,k,1))*n0g*abs(vt2g-vt2r)*(denr/den(i,k))   &
                        *acrfac
            pgacr(i,k) = min(pgacr(i,k),qrs(i,k,1)/dtcld)
          endif
!-------------------------------------------------------------
