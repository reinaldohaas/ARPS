...
!            call mprintf(.true.,ERROR,'MAP_INIT')
            call arpsstop('ERROR: MAP_INIT',1)
         END IF
...
!         call mprintf(.true.,ERROR,'LATLON_TO_IJ')
            call arpsstop('ERROR: LATLON_TO_IJ',1)
      ENDIF
...
!         call mprintf(.true.,ERROR,'IJ_TO_LATLON')
            call arpsstop('ERROR: IJ_TO_LATLON',1)
      ENDIF
      SELECT CASE (proj%code)
...
!      lon = deltalon + proj%stdlon
   ...
   SUBROUTINE llij_rotlatlon_org(lat, lon, proj, i, j)
...
!!! Added next line as a Kludge - not yet understood why needed
  if (ncol .le. 0) ncol=ncol-1
   ...
   END SUBROUTINE llij_rotlatlon_org

   SUBROUTINE llij_rotlatlon(lat, lon, proj, i, j)
!
! return i,j in full E-Grid, so grid point in X direction is two
! times larger.
!
      IMPLICIT NONE
      ...
      INTEGER         :: midrow, midcol
      INTEGER(KIND=8) :: dloni
      REAL(KIND=HIGH) :: dlon, dphd, dlmd
      REAL(KIND=HIGH) :: pi, d2r, r2d
      REAL(KIND=HIGH) :: dlonr, rlatr, rlatrot, rlonrot, xlatcr
...
!      dlon = (mod(dlon+900.,360.)-180.)
      dloni = NINT((dlon+900.)*100)    ! To keep 2 decimal after point
      dloni = MOD(dloni,36000) - 18000
      dlon = dloni/100.0D0

      dlonr = dlon*d2r
      rlatr = lat*d2r

      xlatcr = proj%lat1*d2r

      !
      ! convert lat/lon to the rotated system
      !
      IF (cos(xlatcr) /= 1.0) THEN
        rlatrot = asin ( cos(xlatcr)*sin(rlatr)-sin(xlatcr)*cos(rlatr)*cos(dlonr) )
        rlonrot = atan2( cos(rlatr )*sin(dlonr),                        &
                         cos(xlatcr)*cos(rlatr)*cos(dlonr)+sin(xlatcr)*sin(rlatr) )
      ELSE
        rlatrot = rlatr
        rlonrot = dlonr
      END IF
      ...
      dphd = d2r*proj%phi/REAL((proj%jydim-1)/2)
      dlmd = d2r*proj%lambda/REAL(proj%ixdim-1)

      j = rlatrot / dphd + midrow
      i = rlonrot / dlmd + midcol
...
!         call mprintf(.true.,ERROR,'Gaussian_latitude_computation')
            call arpsstop('ERROR: Gaussian_latitude_computation',1)
      END IF
...
!            call mprintf(.true.,ERROR,'Gee_no_bounding_lats_Gaussian')
            call arpsstop('ERROR: Gee_no_bounding_lats_Gaussian',1)
         END IF
