...
!  PURPOSE:
!
!  Convert the F77 include files into F90 modules
!
!-----------------------------------------------------------------------
!
!  AUTHOR: Alan Miller
...
!    05/01/1999(Yanming Li)
!    fixed runtime segmentation fault problem on origin(IRIS)/Cray
!    Modified to output f90 code of ARPS f90 standards
!    fixed comment/continuation problem
!    Added MODULE name for new f90 code
!
!-----------------------------------------------------------------------
!
!  Usage:
!    compile: f90 convert_inc_to_mod.f90
...
DO
  ...
  END IF
  OPEN(8, file=f77_name, status='old')

  pos = INDEX(f77_name, '.')

  statement = f77_name(1:pos-1)  !get a Module name
...
!     Change C, c or * in column 1 to !
    ...
    pos = INDEX( current%text, '.INC' )
    if ( pos > 0 ) THEN
      current%text = current%text(1:pos-1) //'.F90'// current%text(pos+4:)
    endif
    ...
    ELSE IF (current % label(1:1) == '!') THEN
      current % label = ' '     !! tried to modify ' ' with '!'     
    ELSE
...
!     If found, add an ampersand at the end of the previous line.
    ...
    IF (current % label == '     ' .AND. current % text(6:6) /= ' ' .AND.  &
        current % text(1:1) /= '!') THEN
       pos = INDEX(last_line%text(2:), '!')
      if(pos > 0 ) THEN
        last_line%text = last_line%text(1:pos-1)//'& '//last_line%text(pos:)
        current%text(6:6) = ' '
      ELSE
        last = LEN_TRIM(last_line % text)
        last_line % text(73:73) = '&'
        current % text(6:6) = ' '
      ENDIF
    ELSE
      data_stmnt = .FALSE.
...
!     Remove leading blanks  **!!** the place to modify to get right continuation
...
!--------------------------------------------------------------
!            Replace 'include' with 'use modul'
!            insert 'use module' before implicit none
!--------------------------------------------------------------

! find the beginning of the include lines


statement1 = trim(statement)//'.'
current => head
 do
   pos=INDEX( current%text, trim(statement1))
   posa=INDEX( current%text, trim(statement))
     if( pos > 0) then
     current%text= current%text(1:posa-1)//  &
                 'MODULE '//trim(statement) // &
                 current%text(posa +len_trim(statement)+4:)
     endif
    IF (ASSOCIATED(current, tail)) EXIT
    IF (.NOT. ASSOCIATED(current)) EXIT
   current=>current%next
...
NULLIFY(current)
current => head
 if ( head%text(1:7)=='INTEGER' .OR. &
      head%text(1:7)=='INCLUDE' .OR. &
      head%text(1:4)=='REAL' .OR.   &
      head%text(1:6)=='COMMON' .OR. &
      head%text(1:9)=='PARAMETER' .OR.   &
      head%text(1:9)=='CHARACTER') THEN
  CALL insert_and_moveto_newline( current )
  current%text = head%text 
  head%text = ' '
 ENDIF
...
current => head
   ...
   next_line => current%next
    ...
    if ( current%text(1:20)=='!     ##############') then
       if( next_line%text(2:)==' ')then 
     CALL insert_and_moveto_newline( current )
     current%text = ' '
     CALL insert_and_moveto_newline( current )
     current%text = 'MODULE '//statement
     CALL insert_and_moveto_newline( current )
     current%text = ' '
     CALL insert_and_moveto_newline( current )
     current%text = 'IMPLICIT NONE'
     CALL insert_and_moveto_newline( current )
     current%text = 'SAVE'
     CALL insert_and_moveto_newline( current )
     current%text = '  '
    ...
    if ( next_line%text(1:7)=='INTEGER' .OR. &
         next_line%text(1:4)=='REAL' .OR.   &
         next_line%text(1:6)=='COMMON' .OR. &
         next_line%text(1:9)=='PARAMETER' .OR.   &
         next_line%text(1:9)=='CHARACTER') THEN
     CALL insert_and_moveto_newline( current )
     current%text = ' '
     CALL insert_and_moveto_newline( current )
     current%text = 'MODULE '//statement
    ...
    if ( tail%text(1:1) =='!' ) then
      tail%text = 'END MODULE '//statement
    else
      CALL insert_and_moveto_newline( tail )
      tail%text = ' '
      CALL insert_and_moveto_newline( tail )
      tail%text = 'END MODULE '//statement
    endif
!
! Delete  lines of common blocks
   ...
   pos = 0
    ...
    do
     next_line=>current
     if(current%text(1:6)== 'COMMON'.AND.INDEX(current%text,'&')==0)then
...
!   Find 'include *.inc' and move them  right before implicit none
    ...
    do
     ...
     if(ASSOCIATED(current1, tail) ) exit
...
!     Comment line (unchanged)
  ...
  current=>head
  do
    if (current%text(1:1) == '!' .OR. INDEX(current%text, 'MODULE') &
        > 0 ) THEN
    ! WRITE(9, '(a)') TRIM(current % text)
     current%text = TRIM(current % text)
  ...
  do
    pos = INDEX(current%text, '&')
    if (INDEX(current%text, '!') <= 0 .AND. pos > 0 ) THEN
    ! WRITE(9, '(a)') TRIM(current % text)
     current%text(pos:pos) = ' '
     current%text(73:73) = '&'
     WRITE(9, '(a)') TRIM(current % text)
...
READ(current % text(pos2+1:length), *, ERR=99) lab1, lab2, lab3
