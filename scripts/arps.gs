...
************************************************************************
    ...
    if ( _ctlfile = '' )
        say " "
        say "Usage: run arps.gs ARPS_control_filename"
        say " "
        return
    else
        'open ' _ctlfile
...
************************************************************************
    ...
    'draw string 5.5 5.5 Display ARPS Data'
    'set string 7 bc 6'
...
************************************************************************
    ...
    if ( _vars%_vnpl = 0 )
      ...
      _varln = (_vars-_vars%_vnpl)/_vnpl + 1
    endif
...
*   'set gxout shaded'
    ...
    'd ptprt'
*   dummy = colorbars()
    'draw title Initial ptprt at z='height
...
************************************************************************
    ...
    while( _vchoi!=0 )
        ...
        if ( _mchoi.2=0 & _mchoi.3=0 & _mchoi.4=0 & _mchoi.5=0 & _mchoi.6=0 & _mchoi.7=0 )
            'clear'
            dummy = colors() 
        endif
        ...
        if ( _mchoi.2=2 | _mchoi.3=2 | _mchoi.4=2 | _mchoi.5=2 | _mchoi.6=2 | _mchoi.7=2 )
            drawflg = 0
        endif
...
function drawf()
    ...
    if ( i=_vchoi & i=_vnum+1 )
      titl = 'Total horizontal wind speed'
      vara = 'mag(u,v)'
      prnm = hwin
    endif
    i = i + 1

    if ( i=_vchoi & i=_vnum+2 )
      gxtype = vector
      if ( mchoi.1!=0 )
        titl = 'u-v field'
        vara = 'u;v'
    ...
    if ( i=_vchoi & i=_vnum+3 )
      gxtype = vector
      if ( mchoi.1!=0 )
        titl = 'uprt-vprt field'
        vara = 'uprt;vprt'
        prnm = uvprt
      else
        if ( mchoi.2!=0 )
    ...
    if ( i=_vchoi & i=_vnum+4 )
      gxtype = stream
      if ( mchoi.1!=0 )
        titl = 'u-v streamlines'
        vara = 'u;v'
        prnm = uvstrm
      else
        if ( mchoi.2!=0 )
          titl = 'u-w streamlines'
          vara = 'u;w'
          prnm = uwstrm
        else
          if ( mchoi.3!=0 )
            titl = 'v-w streamlines'
            vara = 'v;w'
            prnm = vwstrm
          endif
        endif
    ...
    if ( i=_vchoi & i=_vnum+5 )
      gxtype = stream
      if ( mchoi.1!=0 )
        titl = 'uprt-vprt streamlines'
        vara = 'uprt;vprt'
        prnm = uvprtstrm
      else
        if ( mchoi.2!=0 )
          titl = 'uprt-w streamlines'
          vara = 'uprt;w'
          prnm = uwprtstrm
        else
          if ( mchoi.3!=0 )
            titl = 'vprt-w streamlines'
            vara = 'vprt;w'
            prnm = vwprtstrm
          endif
        endif
...
* function qfield(ch,time,levl,hor,xzc,yzc,tse,zpr,zoom,anm,ovl,prn)
function qfield()
    ...
    x0 = _rbtnx0 + _rbtnlen/2
    y0 = _rbtny0+_rbtnhgt+0.3

    'set strsiz 0.12 0.18'
    'set string 1 c 6'
    'draw string 'x0' 'y0' Height (km)'
    'set string 1 c 3'
    ...
    if ( _nz-1 <= 30 )
      ...
      while (i < nz1)
        ...
        dummy = button(x0,y0,x1,y1,push,1,5,0.12,0.08,az)
        i = i + 1
      endwhile
    else
      nz1 = _nz1/2
...
* Check if the click point is on level menu
...
*         endif
          ...
          if ( _mchoi.4=1 )
            ...
            if ( cxlb = 'Lon' & cylb = 'Lat' )
              ...
              iz = _level-1
              iz = _lev.iz
              _plab = 'x='cx' km, y='cy' km, z='_lev.iz' km'
            else
              _mchoi.4 = 0
            endif
            break
          ...
          if ( _mchoi.5=1 )
            ...
            if ( cxlb = 'Lon' & cylb = 'Lat' )
              ...
              'set t '_time
              it = (_time-1)*_tint
              _plab = 'x='cx' km, y='cy' km, t='it' '_tunt
              _ylab = 'Z (km)'
            else
              _mchoi.5 = 0
            endif
